...
WHAT THIS SCRIPT DOES (DESIGN CONTRACT)
---------------------------------------
✅ Runs as a MODULE:
   - import and call run_scan(cfg) to receive structured results (no printing, no sleeping)
...
✅ Keeps "educator-grade" comments and redundant prints:
   - In trading systems, redundant printing is a feature: it makes errors obvious.

KEY FIX (requested)
-------------------
Your original script still produced usable "top movers" even outside RTH. The refactor initially relied too
heavily on ONE scanner code (TOP_PERC_GAIN). IBKR can cancel that subscription outside market hours.

We now implement a multi-tier scanner fallback:
  Tier 1: TOP_PERC_GAIN  (best during market hours)
  Tier 2: HOT_BY_VOLUME  (more reliable premarket/after-hours)
  Tier 3: MOST_ACTIVE    (last-resort, usually always returns something)

So the script ALWAYS returns symbols if IBKR is connected and the farms are up.

FLOAT NOTE
----------
IBKR does not reliably provide free float. This script uses an optional local float cache:
  floats_cache.json  -> {"INBS": 8320000, "NUAI": 27400000, ...}

If float is missing, float_category becomes "UNKNOWN".
Ross 5 Pillars filter v1 is conservative: UNKNOWN float FAILS by default.
(You can relax later once the foundation is stable.)
...
class RossPillarsConfig:
    ...
    IMPORTANT:
    Different traders use different thresholds (Ross sometimes says RVOL 5x+).
    We'll keep a slightly relaxed default (2x) so you still see candidates,
    then tighten later once your float cache and news logic mature.
    """
    ...
    min_rvol: float = 2.0           # Ross: wants unusual volume (many use 5x; tune later)
...
class ScannerConfig:
    ...
    # IBKR scanner fallback: ensures symbols even outside RTH
...
# ============================
# Section 7: RSS fetching (async) — safe + similar to your original style
# ============================
...
def fetch_all_feeds(cfg: ScannerConfig, rss_urls: List[str]) -> Dict[str, List[Dict[str, Any]]]:
    """
    Runs async RSS fetcher in a safe event loop wrapper.
    Keeps your standalone behavior consistent across environments.
    """
    if not rss_urls:
        return {}
    try:
...
# ============================
# Section 8: IBKR scan + enrichment (gap, % change, rvol, float, spread)
# ============================
...
def _ibkr_request_scanner_with_fallback(ib: IB, cfg: ScannerConfig) -> Tuple[List[Any], str]:
    """
    KEY IMPROVEMENT:
    Try multiple scan codes so we still get symbols premarket/after-hours/holidays.

    Returns: (scan_data, scan_code_used)
    """
    last_exception = None
    for code in cfg.ibkr_scan_codes:
        try:
            LOGGER.info("Requesting IBKR scanner: %s (rows=%d)", code, cfg.ibkr_top_gainers_count)
            ...
            if scan_data:
                LOGGER.info("Scanner %s returned %d rows", code, len(scan_data))
                return scan_data, code
            LOGGER.warning("Scanner %s returned 0 rows; trying next fallback.", code)
        except Exception as e:
            # IBKR sometimes logs “cancellation” as an error; we treat it as non-fatal.
            LOGGER.warning("Scanner %s failed (non-fatal). Reason: %s", code, e)
            last_exception = e

    # If we get here, all scan codes failed
    if last_exception:
        LOGGER.error("All IBKR scanners failed. Last error: %s", last_exception)
    else:
        LOGGER.error("All IBKR scanners failed (no exception, no data).")
    return [], "NONE"

def fetch_ibkr_symbols_enriched(cfg: ScannerConfig, floats_cache: Dict[str, int]) -> Dict[str, Dict[str, Any]]:
    """
    Fetch symbols via IBKR scanner (with fallback) and enrich with:
      - last_price, open, prev_close
    ...
    """
    ib = IB()
    client_id = int(time.time()) % 9999  # unique-ish per run
    try:
    ...
    try:
        # --- Scanner request with fallbacks ---
        scan_data, scan_code_used = _ibkr_request_scanner_with_fallback(ib, cfg)
        if not scan_data:
            return {}

        # Build metadata first from scanner rows (contracts exist even if scanner later cancels)
        ...
        # Request tickers for enrichment
        if contracts:
            ...
            for t in tickers:
                ...
                # % change vs prev close (intraday momentum)
                percent_change = None
                if prev_close and last_price:
                    percent_change = ((last_price - prev_close) / prev_close) * 100

                # gap % vs prev close (premarket interest proxy)
                gap_percent = None
                if prev_close and open_price:
                    gap_percent = ((open_price - prev_close) / prev_close) * 100

                # Relative volume: how "real" and "crowded" the move is
...
def normalize_hotword_list(meta: Dict[str, Any]) -> List[str]:
    ...
    - symbol + company name tokens
    - Later you can improve with synonyms, tickers in titles, entity resolution.
    """
    candidates = [
        meta.get("symbol", ""),
        meta.get("longName", ""),
...
def trade_suggestion_for_symbol(avg_sent: float, is_spike: bool, heat_5m: int, keyword_score: float, top_domains: List[str]) -> Tuple[str, str]:
    ...
    # Not an auto-trader; just a hint for humans
...
def build_entries(
    ...
    for sym, meta in symbols_meta.items():
        ...
        entries.append({
            ...
            # Market data (Ross pillars input)
...
def passes_ross_five_pillars(cfg: ScannerConfig, e: Dict[str, Any]) -> bool:
    """
    Ross 5 Pillars (v1), with explicit variable mapping:

    1) Relative Volume threshold (Ross asks: "Is it unusual volume?")
       - We PRINT: RVOL:{relative_volume}×
       - We FILTER: e["relative_volume"] >= cfg.ross.min_rvol

    2) Gap % / Daily momentum (Ross asks: "Is it a gapper / strong mover?")
       - We PRINT: Gap:{gap_percent}%
       - We FILTER: e["gap_percent"] >= cfg.ross.min_gap_percent

    3) News catalyst (Ross asks: "Is there a reason?")
       - We PRINT: Total Articles / Unique Articles / Regions
       - We FILTER: total_headlines > 0 (or strong RVOL as a proxy)

    4) Price range (Ross asks: "Is it in the sweet spot? $2-$20")
       - We PRINT: Price:${last_price}
       - We FILTER: min_price <= last_price <= max_price

    5) Low float (Ross asks: "Is supply constrained?")
       - We PRINT: Float:{formatted} (LOW/MID/HIGH/UNKNOWN)
       - We FILTER: float_shares <= max_float
       - v1 conservative: float missing => FAIL
    """
    ross = cfg.ross
...
# ============================
# Section 12: PRINTERS (print-only)
# ============================
...
def print_ross_line(e: Dict[str, Any]):
    """
    Your agreed Ross “one-line” display:
    INBS | 🔥 | Gap:..% | RVOL:..× | Float:.. (LOW) | Price:$.. | Total Articles: .. | Unique Articles: .. | Vel10m:.. | Sent:+.. | Score:.. | Region Count: ..
    """
    sym = e.get("symbol", "")
    ...
    # News flag (simple): Y if any articles matched
...
def print_general_verbose(cfg: ScannerConfig, entries: List[Dict[str, Any]]):
    ...
    for e in entries_sorted:
        ...
        print(
            ...
            f"IBScan:{e.get('ibkr_scan_code_used','')}"
        )
...
def print_filtered_compact(cfg: ScannerConfig, entries: List[Dict[str, Any]]):
    ...
    if not filtered_sorted:
        print("No symbols passed Ross 5 pillars (v1). (Likely float cache missing.)")
        print("=" * 70)
        return
...
def run_scan(cfg: ScannerConfig) -> Dict[str, Any]:
    """
    Core scanner engine. Safe to call from a larger trading system.
    ...
    # IBKR scan + market enrichment (with scan fallback)
    symbols_meta = fetch_ibkr_symbols_enriched(cfg, floats_cache)
    if not symbols_meta:
        raise RuntimeError("No symbols returned from IBKR scanners (all fallbacks failed).")

    # RSS fetch
    feeds_map = fetch_all_feeds(cfg, rss_urls)

    # Aggregate news matches
    now_val = now_ts()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-ib", action="store_true", help="Disable IBKR (engine will refuse to run)")
    parser.add_argument("--top", type=int, default=50, help="IBKR top rows (default 50)")
