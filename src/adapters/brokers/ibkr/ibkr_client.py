...
class IbkrClient(EWrapper, EClient):
    ...
    def __init__(
        ...
        self._order_warnings: Dict[int, List[Tuple[int, str]]] = {}
    ...
    def get_order_warnings(self, order_id: int) -> List[Tuple[int, str]]:
        return list(self._order_warnings.get(order_id, []))
    ...
    def error(self, reqId: int, errorCode: int, errorString: str):  # type: ignore[override]
        warning_only = int(errorCode) == 2109
        if reqId >= 0:
            self._errors[reqId] = (errorCode, errorString)
            if reqId in self._order_status_events:
                existing = self._order_status.get(reqId, {})
                if warning_only:
                    warnings = self._order_warnings.setdefault(reqId, [])
                    warnings.append((errorCode, errorString))
                    ...
                    print(
                        f"[IBKR][WARN] order_id={reqId} code={errorCode} message={errorString}"
                    )
                else:
        ...
        self._order_status[orderId] = {
            **existing,
            ...
            "broker_warnings": [
                ...
                for code, message in self._order_warnings.get(orderId, [])
            ],
        }
