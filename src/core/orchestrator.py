...
from src.scanner.scanner_runner import build_scanner_candidates, run_scanner_cycle
...
class CoreOrchestrator:
    ...
    def _run_once_inner(self) -> bool:
        ...
        row_validations: Dict[str, dict] = {}
        scanner_candidates: List[ScannerCandidate] = []
        try:
            scanner_artifact = run_scanner_cycle(mode="integrated")
            scanner_rows = scanner_artifact.symbol_rows
            row_validations = scanner_artifact.row_validations
            ...
            self.event_collector.emit(
                ...
                payload={
                    ...
                    "candidates_count": scanner_artifact.candidates_count,
                    "watchlist_count": scanner_artifact.watchlist_count,
                    "symbols": scanner_artifact.symbols,
                },
            )
            print(
                "[SCANNER] Artifact summary "
                f"path={scanner_artifact.artifact_path} "
                f"scanner_rows={len(scanner_rows)} "
                f"scanner_candidates={len(scanner_candidates)} "
                f"watchlist_count={scanner_artifact.watchlist_count}"
            )
        except Exception as exc:
            print(f"[SCANNER] Integrated scanner failed: {exc}")
            self._evaluate_runtime_safety(
                cycle_stage="SCANNER",
                stage_exception=exc,
        ...
        event = self.event_collector.emit(
            ...
            source="Scanner",
            payload={"candidates": len(scanner_candidates or [])}
        )
        print(event)
        if not scanner_candidates:
            print("[SCAN] Scanner returned no candidates — placeholder outcome.")
        else:
            print(f"[SCAN] Scanner produced candidates: {scanner_candidates}")
        ...
        print(
            "[SUMMARY] "
            f"scanner_rows={len(scanner_rows or [])} | "
            f"scanner_candidates={len(scanner_candidates or [])} | "
    ...
    def _evaluate_runtime_safety(
        ...
        scanner_rows: Optional[List[ScannerRow54]] = None,
        scanner_candidates: Optional[List[ScannerCandidate]] = None,
        ...
        if scanner_rows is not None:
            invalid_rows = [
                item for item in scanner_rows if not isinstance(item, ScannerRow54)
            ]
            if invalid_rows:
                violations.append(
                    "Scanner rows contain non-ScannerRow54 elements"
                )
        if scanner_candidates is not None:
            invalid_candidates = [
                item
                for item in scanner_candidates
                if not isinstance(item, ScannerCandidate)
            ]
            if invalid_candidates:
                violations.append(
                    "Scanner candidates contain non-ScannerCandidate elements"
                )
