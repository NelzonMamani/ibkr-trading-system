...
from src.news.rss_batch_runtime import (
    ...
    fast_tier_budget_seconds as _rss_fast_tier_budget_seconds,
    merge_rss_failure_summaries as _rss_merge_rss_failure_summaries,
    news_provider_status as _rss_news_provider_status,
    stage_elapsed_seconds as _rss_stage_elapsed_seconds,
    stage_remaining_seconds as _rss_stage_remaining_seconds,
)
...
def _news_provider_status(summary: Any) -> str:
    return _rss_news_provider_status(summary)
...
def _merge_rss_failure_summaries(*summaries: Any) -> RssFailureSummary:
    return _rss_merge_rss_failure_summaries(*summaries)
...
def _dedupe_bounded_headlines(headlines: Iterable[Headline], max_entries_per_symbol: int) -> List[Headline]:
    return _rss_dedupe_bounded_headlines(headlines, max_entries_per_symbol)
...
def _news_stage_budget_seconds() -> float:
    try:
        return max(0.0, float(get_config("NEWS_TOTAL_BUDGET_S") or 0.0))
    except Exception:
        return 0.0
...
def _news_extended_tier_reserve_fraction() -> float:
    try:
        raw = get_config("NEWS_EXTENDED_TIER_RESERVE_FRACTION")
    except Exception:
        return 0.0
    return _rss_extended_tier_reserve_fraction(raw)


def _news_fast_tier_budget_seconds(total_budget_seconds: float, *, extended_sources_available: bool) -> float:
    return _rss_fast_tier_budget_seconds(
...
def _news_stage_remaining_seconds(stage_deadline_s: float) -> float:
    return _rss_stage_remaining_seconds(stage_deadline_s)


def _news_stage_elapsed_seconds(stage_started_at_s: float) -> float:
