...
from src.strategies.ross_momentum.strategy_policy import (
    StockSelectionSpec as StockSelectionPolicy,
)
...
STOCK_SELECTION_FIELD_ORDER: List[str] = [
    field.name for field in fields(StockSelectionPolicy)
]


def stock_selection_policy_fields():
    return fields(StockSelectionPolicy)
...
def policy_from_config() -> StockSelectionPolicy:
    ...
    return replace(
        ...
        ranking_intent="CONFIG_DEFAULTS",
    )
