...
from src.strategies.pairs_divergence_reversion.strategy import PairsDivergenceReversionStrategy


def _entry(**overrides):
    base = {"symbol": "AAPL"}
    base.update({'pair_symbol': 'MSFT', 'zscore': -2.2, 'spread_reversion_signal': True})
    base.update(overrides)
    return base


def test_strategy_generates_intent_in_sim() -> None:
    strategy = PairsDivergenceReversionStrategy()
