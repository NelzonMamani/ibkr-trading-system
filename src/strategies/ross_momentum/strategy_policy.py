...
POLICY_V2 = StrategyPolicyV2(
    ...
    trigger_model=TriggerModelV2(
        ...
        confirmations=(
            ...
            ConfirmationSpecV2("C_SPREAD_CONFIRM", "Spread must remain within tradable bounds to avoid adverse execution quality."),
            ConfirmationSpecV2("C_LIQUIDITY_CONFIRM", "Liquidity must be sufficient for intended size without destabilizing fills."),
            ConfirmationSpecV2("C_LEVEL_HOLD", "Breakout/reclaim level should hold after trigger rather than immediately fail."),
            ConfirmationSpecV2("C_BREAK_AND_HOLD", "Valid expansion requires break and hold behavior, not a one-tick poke through level."),
            ConfirmationSpecV2("C_RETEST_CONFIRM", "Retest behavior should confirm continuation when setup logic expects a pullback check."),
            ConfirmationSpecV2("C_NO_PARABOLIC_EXHAUSTION", "Avoid new entries when parabolic exhaustion context indicates late-stage blowoff risk."),
            ConfirmationSpecV2("C_DATA_QUALITY_CONFIRM", "Data/tape quality must be reliable enough to trust trigger and risk signals."),
    ...
    candle_and_volume_evidence=CandleAndVolumeEvidenceModelV2(
        evidence_tags=(
            ...
            "SHOOTING_STAR",
            "HAMMER",
            "LONG_UPPER_WICK",  # Ross terminology for long upper wick rejection; equivalent to TOPPING_TAIL / shooting-star warning.
            "MARUBOZU",
            "ENGULFING",
...
# ---------------------------------------------------------------------------
# Legacy compatibility surface (v1 API) retained to avoid regressions while
# canonical policy source remains StrategyPolicyV2 in this module.
# ---------------------------------------------------------------------------
...
class MicroPullbackSpec:
    ...
    red_volume_pause_ratio: Optional[float] = None
...
class RossMomentumPolicy:
    ...
    version: str = "v1"
    timeframe_opening: TimeframePlan = TimeframePlan("DAILY", "5MIN", "1MIN", "10SEC")
    timeframe_midday: TimeframePlan = TimeframePlan("DAILY", "5MIN", "1MIN", "10SEC")
    timeframe_late_day: TimeframePlan = TimeframePlan("DAILY", "15MIN", "5MIN", "1MIN")
...
def select_watchlist(
    ...
    session_allowlist = {normalize_session_label(s).upper() for s in spec.session_allowlist}
    eligible: list[CandidateMetrics] = []
    for observation in observations:
        reasons: list[str] = []
        raw = (observation.session_label or "").strip()
        session_label = normalize_session_label(raw).upper() if raw else ""
        ...
        if reasons:
            observation = replace(observation, drop_reasons=list({*observation.drop_reasons, *reasons}))
        else:
            eligible.append(observation)

    ranked = sorted(
        eligible,
        key=lambda row: (row.rank_score or 0.0, row.pct_change or 0.0, row.dollar_volume or 0.0),
        reverse=True,
    )
...
def timeframe_plan_for_session_phase(policy: RossMomentumPolicy, session_phase: str) -> TimeframePlan:
    return timeframe_plan_for_mode(policy, mode_for_session_phase(session_phase))


def stock_selection_policy_for_session_phase(policy: RossMomentumPolicy, session_phase: str) -> StockSelectionSpec:
