...
def _inputs(setup_id: str, positive: bool) -> PatternInputs:
    base = PatternInputs(
        ...
        levels=LevelSet(premarket_high=10.5, hod=10.9, prior_close=9.8, key_levels={'pivot':10.4}),
    ...
    positive_map = {
        ...
        'MICRO_PULLBACK': replace(base, candles=_candles([(10,10.2,9.99,10.15,900),(10.15,10.35,10.14,10.3,1000),(10.3,10.32,10.22,10.24,800),(10.24,10.25,10.2,10.22,780),(10.22,10.23,10.18,10.2,760),(10.2,10.22,10.18,10.19,740),(10.19,10.42,10.18,10.4,1200)]), indicators=IndicatorSet(ema9=10.2, ema20=10.1, vwap=10.18)),
        ...
        'HALT_RESUME': replace(base, candles=_candles([(10.1,10.8,10.0,10.4,1000),(10.4,10.6,10.25,10.45,800),(10.45,10.62,10.35,10.5,820),(10.5,10.9,10.48,10.86,1400)]), levels=LevelSet(premarket_high=10.5, hod=10.9, prior_close=9.8, key_levels={'HALT_RESUME_LEVEL':10.62})),
        ...
        'OPENING_DRIVE': replace(base, session_context=SessionContext.REGULAR, candles=_candles([(10,10.1,9.98,10.05,900),(10.05,10.12,10.0,10.08,900),(10.08,10.18,10.05,10.16,910),(10.16,10.25,10.12,10.22,920),(10.22,10.35,10.2,10.32,1100)])),
        ...
        'HOD_BREAK': replace(base, session_context=SessionContext.REGULAR, candles=_candles([(10.0,10.1,9.98,10.02,800),(10.02,10.2,10.0,10.16,850),(10.16,10.24,10.12,10.2,860),(10.2,10.32,10.18,10.28,900),(10.28,11.02,10.26,10.96,1400)]), levels=LevelSet(premarket_high=10.5, hod=10.9, prior_close=9.8)),
        'EMA_PULLBACK': replace(base, candles=_candles([(10,10.1,9.98,10.02,800),(10.02,10.18,10.0,10.16,850),(10.16,10.2,10.08,10.1,820),(10.1,10.14,10.05,10.08,800),(10.08,10.22,10.07,10.2,1100)]), indicators=IndicatorSet(ema9=10.12, ema20=10.08, vwap=10.08)),
        'VWAP_PULLBACK': replace(base, candles=_candles([(10,10.1,9.98,10.02,800),(10.02,10.18,10.0,10.16,850),(10.16,10.2,10.08,10.1,820),(10.1,10.14,10.05,10.08,800),(10.08,10.22,10.07,10.2,1100)]), indicators=IndicatorSet(ema9=10.1, ema20=10.08, vwap=10.1)),
        ...
        'SECOND_PULLBACK': replace(base, candles=_candles([(10,10.18,9.98,10.16,900),(10.16,10.14,10.06,10.08,820),(10.08,10.24,10.07,10.2,900),(10.2,10.18,10.1,10.12,800),(10.12,10.14,10.05,10.08,780),(10.08,10.3,10.07,10.28,1200)])),
    ...
    return candidate if positive else replace(candidate, candles=candidate.candles[:-1])
...
def test_every_trade_ready_setup_has_positive_trigger_proof() -> None:
    ...
    for setup_id in _trade_ready_ids():
        ...
        elif (result.non_entry_classification or "").upper() not in {"EXIT", "AVOID", "CAUTION"}:
            assert result.trigger_type, f"{setup_id} missing trigger_type"
            assert result.trigger_level is not None, f"{setup_id} missing trigger_level"
            assert result.entry_reference and result.stop_reference and result.invalidation_reference
