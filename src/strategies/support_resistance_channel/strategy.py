"""Support Resistance Channel strategy adapter."""
...
class SupportResistanceChannelStrategy(BaseStrategy):
    ...
    trader_type = "SUPPORT_RESISTANCE_CHANNEL"
    ...
    def process_watchlist(self, *, watchlist: List[object], snapshots: dict, session_label: str, timestamp_utc: str, mode: RunMode, session_phase: str) -> List[TradeIntent]:
        ...
        return [
            TradeIntent(
                ...
                rationale="Support/resistance channel score qualified deterministic breakout-continuation setup.",
    ...
    def _qualifies(self, *, symbol: str, timestamp_utc: str, session_phase: str) -> bool:
        del session_phase
        channel_score = self._score(f"sr-channel:{symbol}:{timestamp_utc}", scale=10_000)
        return channel_score >= 0.45
