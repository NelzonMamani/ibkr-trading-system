"""Volatility Expansion strategy adapter."""
...
class VolatilityExpansionStrategy(BaseStrategy):
    ...
    trader_type = "VOLATILITY_EXPANSION"
    ...
    def process_watchlist(self, *, watchlist: List[object], snapshots: dict, session_label: str, timestamp_utc: str, mode: RunMode, session_phase: str) -> List[TradeIntent]:
        ...
        return [
            TradeIntent(
                ...
                rationale="Volatility expansion proxy crossed deterministic threshold.",
    ...
    def _qualifies(self, *, symbol: str, timestamp_utc: str, session_phase: str) -> bool:
        del session_phase
        pseudo_vol = self._score(f"vol-expansion:{symbol}:{timestamp_utc}", scale=10_000)
        return pseudo_vol >= 0.42
