"""Shared trigger evaluator for EMA_PULLBACK family."""
...
def evaluate_ema_pullback_trigger(payload, values):
    ...
    if len(candles) < 2 or trigger_level is None or stop_level is None or invalidation_level is None:
        ...
        print(f"[TRIGGER][EMA_PULLBACK] fired=False reason={out['trigger_reason']}")
        return out
    ...
    out = {**base, "trigger_state": "FIRED", "trigger_ready_now": True, "trigger_reason": "ema_pullback_breakout_confirmed"}
    print(f"[TRIGGER][EMA_PULLBACK] fired=True reason={out['trigger_reason']}")
    return out
