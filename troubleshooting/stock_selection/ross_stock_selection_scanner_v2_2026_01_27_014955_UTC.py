...
def build_output_path(run_timestamp_utc: str) -> Path:
    ...
    filename = f"ross_stock_selection_v2_{run_timestamp_utc}.txt"
...
def main():
    ...
    emit_line("Version: v2", output_lines)
    ...
    for row in scanner_results:
        ...
        spread_abs, spread_pct = derive_spread_metrics(ticker, last_price)
        liq_shares_min, liq_usd_min = derive_liquidity_metrics(ticker, last_price)
        ...
        rows.append(
            ScannerRow(
                ...
                last=last_close,
                prior=prior_close,
                percent_change=session_percent_change,
