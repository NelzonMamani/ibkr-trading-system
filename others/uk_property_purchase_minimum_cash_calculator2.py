...
# First-time buyer SDLT thresholds
...
# Standard SDLT rates for company / additional property
standard_sdlt_zero_rate_limit = 250_000
standard_sdlt_rate_above_threshold = 0.05
additional_property_surcharge_rate = 0.03  # 3% surcharge
...
def main():
    ...
    print(f"Deposit = 5% × {format_currency(property_price)} = {format_currency(first_time_buyer_deposit)}")

    print("\n2. Mortgage Amount Required")
    print(f"Mortgage = Property Price − Deposit = {format_currency(first_time_buyer_mortgage)}")

    # Stamp Duty
    print("\n3. Stamp Duty Land Tax (SDLT)")
    if property_price <= first_time_buyer_sdlt_zero_rate_limit:
        sdlt_first_time = 0
        print(f"Property price ≤ {format_currency(first_time_buyer_sdlt_zero_rate_limit)} → SDLT = £0.00")
    else:
        taxable_amount = min(property_price, first_time_buyer_sdlt_upper_limit) - first_time_buyer_sdlt_zero_rate_limit
        sdlt_first_time = taxable_amount * first_time_buyer_sdlt_rate_above_threshold
        print(f"Taxable Amount = {format_currency(property_price)} − "
              f"{format_currency(first_time_buyer_sdlt_zero_rate_limit)} = {format_currency(taxable_amount)}")
        print(f"SDLT = 5% × {format_currency(taxable_amount)} = {format_currency(sdlt_first_time)}")
    ...
    # Total cash
    total_cash_first_time = first_time_buyer_deposit + sdlt_first_time + additional_costs
    ...
    print("\n1. Deposit Calculation")
    print(f"Deposit = 25% × {format_currency(property_price)} = {format_currency(company_deposit)}")

    print("\n2. Mortgage Amount Required")
    print(f"Mortgage = Property Price − Deposit = {format_currency(company_mortgage)}")

    # Stamp Duty with surcharge
    print("\n3. Stamp Duty Land Tax (SDLT)")
    if property_price <= standard_sdlt_zero_rate_limit:
        taxable_amount_company = 0
        base_sdlt = 0
        print(f"Property price ≤ {format_currency(standard_sdlt_zero_rate_limit)} → Base SDLT = £0.00")
    else:
        ...
        print(f"Taxable Amount = {format_currency(property_price)} − "
              f"{format_currency(standard_sdlt_zero_rate_limit)} = {format_currency(taxable_amount_company)}")
        print(f"Base SDLT = 5% × {format_currency(taxable_amount_company)} = {format_currency(base_sdlt)}")

    surcharge = property_price * additional_property_surcharge_rate
    print(f"Surcharge = 3% × {format_currency(property_price)} = {format_currency(surcharge)}")

    sdlt_company = base_sdlt + surcharge
    print(f"Total SDLT = Base SDLT + Surcharge = {format_currency(base_sdlt)} + "
          f"{format_currency(surcharge)} = {format_currency(sdlt_company)}")

    # Total cash
    total_cash_company = company_deposit + sdlt_company + additional_costs
