...
# ============================
# Section 7: News Truth (Phase 3A/3B/3C)
# - Presence: total/unique/replicated
# - Velocity: counts in last 10m and last 60m
# - Attribution: region + top sources (best-effort)
# - Freshness: newest headline age (minutes)
# - Time distribution: bucket counts (stored inside score_components_breakdown["news_time_distribution"])
# - Clickable headlines: OSC 8 hyperlinks where supported (falls back to plain text)
# ============================

NEWS_TIME_BUCKETS: List[Tuple[str, int]] = [
    ("0–1m", 1),
    ("1–5m", 5),
    ("5–10m", 10),
    ("10–20m", 20),
...
def _safe_now_utc() -> datetime:
    return datetime.now(timezone.utc)


def _parse_entry_time_utc(entry: Any) -> Optional[datetime]:
    """Best-effort parse for RSS entry timestamps to aware UTC datetime."""
    for attr in ("published_parsed", "updated_parsed"):
        tm = getattr(entry, attr, None)
        if tm:
            try:
                # feedparser returns time.struct_time in UTC for most feeds; treat as UTC.
...
def _region_from_domain(domain: str) -> str:
    """Crude region heuristic based on TLD; designed to be stable and non-failing."""
    if not domain:
        return "Unknown"
...
def _format_clickable(title: str, url: str) -> str:
    """Return an OSC-8 clickable hyperlink if supported, else a plain fallback."""
    t = (title or "").strip() or "(untitled)"
    u = (url or "").strip()
    if not u:
        return t

    # Windows Terminal sets WT_SESSION; many modern terminals support OSC 8.
    supports = bool(os.environ.get("WT_SESSION")) or os.environ.get("TERM_PROGRAM") in {
        "iTerm.app",
        "WezTerm",
        "Apple_Terminal",
    }
    if supports:
        # OSC 8: \x1b]8;;URL\x07TITLE\x1b]8;;\x07
        return f"\x1b]8;;{u}\x07{t}\x1b]8;;\x07"
    return f"{t} ({u})"


def fetch_news_truth_for_symbol(symbol: str) -> Dict[str, Any]:
    """Fetch per-symbol news signal using Google News RSS search.

    This is intentionally best-effort and non-fatal. If dependencies are missing
    (requests/feedparser) or the request fails, returns zeros.
    """
    sym = (symbol or "").strip().upper()
    if not NEWS_ENABLED or not sym:
        return {}
    ...
    if feedparser is None or requests is None:
        _NEWS_CACHE[sym] = result
        return result

    # Google News RSS query is the most widely available free option.
    # Query includes "stock" to reduce irrelevant acronyms.
    ...
    try:
        resp = requests.get(
            ...
            timeout=NEWS_REQUEST_TIMEOUT_S,
        )
        resp.raise_for_status()
        feed = feedparser.parse(resp.text)
        entries = list(getattr(feed, "entries", []) or [])[:NEWS_MAX_ENTRIES_PER_SYMBOL]
    ...
    now = _safe_now_utc()
    cutoff = now.timestamp() - NEWS_LOOKBACK_HOURS * 3600

    # Build a list of (time, title, url, source_domain, region)
    items: List[Tuple[Optional[datetime], str, str, str, str]] = []
    for it in entries:
        title = getattr(it, "title", "") or ""
        link = getattr(it, "link", "") or ""
        ts = _parse_entry_time_utc(it)
        if ts is not None and ts.timestamp() < cutoff:
            continue
        domain = _domain_from_url(link)
        region = _region_from_domain(domain)
        items.append((ts, title, link, domain, region))

    # Counts and deduplication
    total = len(items)
    norm_titles = [_normalise_title(t) for _, t, _, _, _ in items]
    unique_set = set([t for t in norm_titles if t])
    unique_count = len(unique_set)
    replicated_count = max(0, total - unique_count)

    # Velocity
    def age_min(ts: Optional[datetime]) -> Optional[float]:
        if ts is None:
            return None
        return (now - ts).total_seconds() / 60.0

    ages = [age_min(ts) for ts, *_ in items]
    ages_valid = [a for a in ages if a is not None]
    freshest = min(ages_valid) if ages_valid else None
    vel_10m = sum(1 for a in ages_valid if a <= 10)
    vel_60m = sum(1 for a in ages_valid if a <= 60)

    # Time distribution buckets
    buckets: Dict[str, int] = {label: 0 for label, _ in NEWS_TIME_BUCKETS}
    buckets[">48h"] = 0
    for a in ages_valid:
        placed = False
        for label, max_min in NEWS_TIME_BUCKETS:
            if a <= max_min:
                buckets[label] += 1
                placed = True
                break
    ...
    # Top sources and regions
    domains = [d for *_, d, _ in items if d]
    regions = [r for *_, r in items if r]
    top_sources = [d for d, _ in Counter(domains).most_common(5)]
    regions_unique = sorted(set(regions))

    # Top unique headlines by recency
    # Sort: known timestamps first, newest first.
    items_sorted = sorted(items, key=lambda x: (x[0] is None, x[0] or datetime(1970, 1, 1, tzinfo=timezone.utc)), reverse=True)
    top_unique: List[Dict[str, Any]] = []
    seen = set()
    for ts, title, link, domain, region in items_sorted:
        ...
        top_unique.append(
            {
                ...
                "region": region,
                "age_minutes": round(age_min(ts) or 0.0, 2) if ts is not None else None,
            }
        )
        if len(top_unique) >= 5:
            break

    # Credibility/sentiment/keyword relevance are placeholders for later scoring.
    # For now, credibility is a crude heuristic: more distinct known domains => higher score.
    credibility = min(1.0, len(set(domains)) / 10.0) if domains else 0.0

    result = {
        "news_total_headlines": total,
        "news_unique_headlines": unique_count,
        "news_replicated_headlines": replicated_count,
        "news_velocity_10m": vel_10m,
        "news_velocity_60m": vel_60m,
        "news_spike_indicator": bool(vel_10m >= 5 or vel_60m >= 20),
        "news_freshest_age_minutes": None if freshest is None else round(freshest, 2),
        "news_regions_list": regions_unique,
        "news_region_count": len(regions_unique),
        "news_top_sources_list": top_sources,
        "news_top_source_credibility_score": round(credibility, 3),
        "news_average_sentiment": 0.0,
        "news_keyword_relevance_score": 0.0,
        "news_primary_catalyst_keywords": [],
        "news_top_headlines_list": top_unique,
        "news_time_distribution": buckets,
    }
