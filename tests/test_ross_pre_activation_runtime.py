...
def _pre_bars() -> list[Candle]:
    return [
        ...
        Candle(open=10.50, high=10.60, low=10.45, close=10.58, volume=5200),
    ]


def test_pre_candidate_can_emit_intent_via_pre_activation(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(
    ...
    intents = strategy.process_watchlist(
        ...
        snapshots={"PREX": MarketSnapshot(symbol="PREX", bid=10.57, ask=10.59, last=10.58, volume=200_000, asof_utc=datetime.now(timezone.utc))},
        session_label="PRE",
