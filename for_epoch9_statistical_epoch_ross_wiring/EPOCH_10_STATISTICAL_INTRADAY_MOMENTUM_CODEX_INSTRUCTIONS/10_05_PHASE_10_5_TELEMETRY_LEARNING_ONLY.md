...
Implement telemetry outputs (learning-only) that never affect trading decisions.

## Allowed Files
- `src/strategies/statistical_intraday_momentum/telemetry.py` (create)
- tests

## Requirements
- `build_telemetry(policy, context, features, score, regime) -> dict`
- JSON-serializable, deterministic fields:
  - timestamps, symbol
  - feature values
  - regime and reason codes
  - score components
